...
6. **`config.js` 的 `CONFIG.DEFAULTS` 同時是預設值與 storage schema 的唯一來源** — 字體與位置的預設值只此一份，`content.js` 與 `popup.js` 皆自此取用，兩檔不得再出現預設值字面值。兩個慣用法依賴此物件：讀取一律 `chrome.storage.sync.get(CONFIG.DEFAULTS, cb)`（Chrome 的 `get(object)` 語義即「以該物件為預設值字典」，故回呼取得的 key 集合恆等於 `DEFAULTS` 且每個 key 必定有值，呼叫端不做 truthy 判斷），重置一律自 `DEFAULTS` 取出欲重置的 key 子集為 patch 後 `chrome.storage.sync.set(patch)`（`set()` 為 merge 語義，未列入的 key 保持原值，故局部重置不波及其餘設定；「全部重置」的子集即 `Object.keys(CONFIG.DEFAULTS)`，不隨新增設定項修改）。**新增持久化設定項時，除了於 `DEFAULTS` 加 key，尚須將該 key 歸入 `popup.js` 的 `POSITION_KEYS` 或 `FONT_SIZE_KEYS` 其中一組**，否則該項僅能被「全部重置」清除，三顆按鈕中的兩顆局部重置摸不到它——此為 `DEFAULTS` 唯一來源原則的例外，分組屬 popup UI 語義，無法自 schema 衍生。
